...
namespace timer_detail
{
    ...
    concept Duration = std::same_as<Type, std::chrono::seconds> || std::same_as<Type, std::chrono::milliseconds>
                    || std::same_as<Type, std::chrono::microseconds> || std::same_as<Type, std::chrono::nanoseconds>;
}
...
public:
    [[nodiscard("Timer will immediately destroyed and the output will be not what you wanted"
    ...
    double elapsed() const { return std::chrono::duration_cast<second_type>(clock_type::now() - m_beginning).count(); }
